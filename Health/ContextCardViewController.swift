...
class ContextCardViewController: UIViewController {
  ...
  @IBAction func didPushAddButton(_ sender: UIBarButtonItem) {
    ...
    if activeSensors.count == 0 {
      let alert = UIAlertController(
        title: NSLocalizedString("context_card_no_active_sensors_title", comment: ""),
    ...
    let alert = UIAlertController(
      title: NSLocalizedString("context_card_add_title", comment: ""),
  ...
  @IBAction func didPushRemoveButton(_ sender: UIBarButtonItem) {
    let alert = UIAlertController(
      title: NSLocalizedString("context_card_remove_title", comment: ""),
      message: NSLocalizedString("context_card_remove_msg", comment: ""),
      preferredStyle: .alert)

    alert.addAction(
      UIAlertAction(
        title: NSLocalizedString("context_card_item_remove_all", comment: ""),
        style: .destructive,
        handler: { (action) in
  ...
  func addAccelereomterCard() {
    if let sensor = AWARESensorManager.shared().getSensor(SENSOR_ACCELEROMETER) {
      let contextCard = ScatterChartCard(
        frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 300))
      contextCard.yAxisMax = 6
      contextCard.yAxisMin = -6
      contextCard.granularitySecond = 10
      contextCard.setTodaysChart(
        sensor: sensor, yKeys: ["double_values_0", "double_values_1", "double_values_2"])
  ...
  func addActivityRecognitionCard() {
    if let sensor = AWARESensorManager.shared().getSensor(SENSOR_IOS_ACTIVITY_RECOGNITION) {
      let contextCard = ScatterChartCard(
        frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 300))
      contextCard.setFilterHandler { (key, value) -> [String: Any]? in
        var val = value
        if key == "stationary" {
      ...
      contextCard.setTodaysChart(
        sensor: sensor, yKeys: ["stationary", "walking", "running", "automotive", "cycling"])
  ...
  func addAmbientNoiseCard() {
    if let sensor = AWARESensorManager.shared().getSensor(SENSOR_AMBIENT_NOISE) {
      ...
      // double_frequency
      let frequencyCard = ScatterChartCard(
        frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 300))
  ...
  func addOpenWeatherChart() {
    if let sensor = AWARESensorManager.shared().getSensor(SENSOR_PLUGIN_OPEN_WEATHER) {
      ...
      contextCard.setTodaysChart(
        sensor: sensor, yKeys: ["temperature_min", "temperature", "temperature_max"])
  ...
  func addLocationCard() {
    ...
    if let fusedLocationSensor = AWARESensorManager.shared().getSensor(SENSOR_GOOGLE_FUSED_LOCATION)
      as? FusedLocations
    {
      let contextCard = MapCard(
        frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 400))
      ...
      self.contextCards.append(contextCard)
      self.mainStackView.addArrangedSubview(contextCard)
    } else if let locationSensor = AWARESensorManager.shared().getSensor("locations") as? Locations
    {
      let contextCard = MapCard(
        frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 400))
