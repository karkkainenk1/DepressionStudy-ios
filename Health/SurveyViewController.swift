...
class SurveyViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(updateUI), name: UIApplication.didBecomeActiveNotification,
      object: nil)
  }
  ...
  func radioButtons(
    radioNum: Int, radioTitle: String, radioInstr: String, radioAns: [String], sch: ESMSchedule
  ) {
  ...
  @IBAction func dailyESMPressed(_ sender: Any) {
    ...
    radioButtons(
      radioNum: 1, radioTitle: title, radioInstr: "... tired out for no good reason?",
      radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 2, radioTitle: title, radioInstr: "... nervous?", radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 3, radioTitle: title,
      radioInstr: "... so nervous that nothing could calm you down?", radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 4, radioTitle: title, radioInstr: "... hopeless?", radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 5, radioTitle: title, radioInstr: "... restless or fidgety?", radioAns: ans,
      sch: schdule)
    radioButtons(
      radioNum: 6, radioTitle: title, radioInstr: "... so restless that you could not sit still?",
      radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 7, radioTitle: title, radioInstr: "... depressed?", radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 8, radioTitle: title,
      radioInstr: "... so depressed that nothing could cheer you up?", radioAns: ans, sch: schdule)
    radioButtons(
      radioNum: 9, radioTitle: title, radioInstr: "... that everything was an effort?",
      radioAns: ans, sch: schdule)
  ...
  func startESM(schedule: ESMSchedule, allowClose: Bool) {
    ...
    if allowClose {
      esmViewController.navigationItem.leftBarButtonItem = UIBarButtonItem.init(
        title: "Close", style: UIBarButtonItem.Style.done, target: self,
        action: #selector(dismissESM))
    }
  ...
  func hasAnsweredToday() -> Bool {
    if let context = CoreDataHandler.shared().managedObjectContext {
      ...
      req.entity = NSEntityDescription.entity(
        forEntityName: NSStringFromClass(EntityESMAnswer.self), in: context)

      let sort = NSSortDescriptor.init(key: "timestamp", ascending: false)
      req.sortDescriptors = [sort]
      let resultsController = NSFetchedResultsController.init(
        fetchRequest: req, managedObjectContext: context, sectionNameKeyPath: nil, cacheName: nil)
      ...
      do {
        ...
        if let results = results {
          ...
          if let answer = results.last as? EntityESMAnswer {
            ...
            let latestDate = Date.init(
              timeIntervalSince1970: TimeInterval(Int(truncating: answer.timestamp!) / 1000))
